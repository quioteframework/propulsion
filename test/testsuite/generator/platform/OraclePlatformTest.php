...
";
	...
	public function testGetColumnDDLRealMapsToBinaryFloat()
	{
		$column = new Column('foo');
		$column->getDomain()->copy($this->getPlatform()->getDomainForType(PropulsionTypes::REAL));
		$expected = 'foo BINARY_FLOAT';
		$this->assertEquals($expected, $this->getPlatform()->getColumnDDL($column));
	}

	public function testGetColumnDDLDoubleMapsToBinaryDouble()
	{
		$column = new Column('foo');
		$column->getDomain()->copy($this->getPlatform()->getDomainForType(PropulsionTypes::DOUBLE));
		$expected = 'foo BINARY_DOUBLE';
		$this->assertEquals($expected, $this->getPlatform()->getColumnDDL($column));
	}

	public function testGetColumnDDLJsonMapsToNativeJsonType()
	{
		$column = new Column('foo');
		$column->getDomain()->copy($this->getPlatform()->getDomainForType(PropulsionTypes::JSON));
		$expected = 'foo JSON';
		$this->assertEquals($expected, $this->getPlatform()->getColumnDDL($column));
	}

	public function testGetColumnDDLIdentity()
	{
		...
		$expected = 'id NUMBER GENERATED BY DEFAULT AS IDENTITY NOT NULL';
		$this->assertEquals($expected, $this->getPlatform()->getColumnDDL($column));
	}

	public function testGetColumnDDLIdentityIgnoredWithNamedSequence()
	{
		// A named sequence (<id-method-parameter>) is an explicit opt-out of
		// this column's own implicit identity sequence -- see
		// OraclePlatform::usesNativeIdentity()'s own doc comment. identity="true"
		// is silently ignored in that case, same as PgsqlPlatform's own guard.
		$database = new Database();
		$database->setPlatform($this->getPlatform());
		$table = new Table('foo');
		$table->setIdMethod(IDMethod::NATIVE);
		...
		$column->setNotNull(true);
		$table->addColumn($column);
		$expected = 'id NUMBER NOT NULL';
		$this->assertEquals($expected, $this->getPlatform()->getColumnDDL($column));
	}

	public function testGetAddTableDDLIdentitySkipsLegacySequenceAndTrigger()
	{
		$database = new Database();
		$database->setPlatform($this->getPlatform());
		$table = new Table('foo');
		...
		$this->assertNull($this->getPlatform()->getAddSequencesDDL($table));
		$this->assertSame('', $this->getPlatform()->getAddAutoIncrementTriggerDDL($table));
...
";
	...
	public function testGetDropTableDDLIdentitySkipsSequenceDrop()
	{
		$database = new Database();
		$database->setPlatform($this->getPlatform());
		$table = new Table('foo');
