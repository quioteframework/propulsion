...
class PropulsionJSONParserTest extends TestCase
{
	...
	{
		$parser = new PropulsionJSONParser();
		$this->assertEquals($jsonData, $parser->fromArray($arrayData), 'PropulsionJSONParser::fromArray() converts from ' . $type . ' correctly');
	}
	...
	{
		$parser = new PropulsionJSONParser();
		$this->assertEquals($jsonData, $parser->toJSON($arrayData), 'PropulsionJSONParser::toJSON() converts from ' . $type . ' correctly');
	}
	...
	{
		$parser = new PropulsionJSONParser();
		$this->assertEquals($arrayData, $parser->toArray($jsonData), 'PropulsionJSONParser::toArray() converts to ' . $type . ' correctly');
	}
	...
	{
		$parser = new PropulsionJSONParser();
		$this->assertEquals($arrayData, $parser->fromJSON($jsonData), 'PropulsionJSONParser::fromJSON() converts to ' . $type . ' correctly');
	}
