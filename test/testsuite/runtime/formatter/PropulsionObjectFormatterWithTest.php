...
/**
 * Test class for PropulsionObjectFormatter when Criteria uses with().
 *
 * @author     Francois Zaninotto
 * @version    $Id: PropulsionObjectFormatterWithTest.php 1348 2009-12-03 21:49:00Z francois $
 * @package    runtime.formatter
 */
class PropulsionObjectFormatterWithTest extends BookstoreEmptyTestBase
{
	...
	public function testFindOneWithEmptyLeftJoinOneToMany()
	{
		...
		$b1 = new Book();
		$b1->setTitle('Foo1');
		$b1->setISBN('0000000001');
		$a1->addBook($b1);
		$b2 = new Book();
		$b2->setTitle('Foo2');
		$b2->setISBN('0000000002');
		$a1->addBook($b2);
		$a1->save();
		$con = Propulsion::getConnection(BookPeer::DATABASE_NAME);
	...
	public function testFindOneWithColumn()
	{
		...
		$this->assertEquals('Gunter', $book->getVirtualColumn('AuthorName'), 'PropulsionObjectFormatter adds withColumns as virtual columns');
		$this->assertEquals('Grass', $book->getVirtualColumn('AuthorName2'), 'PropulsionObjectFormatter correctly hydrates all virtual columns');
		$this->assertEquals('Gunter', $book->getAuthorName(), 'PropulsionObjectFormatter adds withColumns as virtual columns');
	}
	...
	public function testFindOneWithColumnAndAlias()
	{
		...
		$this->assertEquals($count, $con->getQueryCount(), 'with() hydrates the related objects to save a query ');
		$this->assertEquals('J.K.', $book->getVirtualColumn('AuthorName'), 'PropulsionObjectFormatter adds withColumns as virtual columns');
	}
	...
	public function testFindOneWithClassAndColumn()
	{
		...
		$this->assertTrue($book instanceof Book, 'withColumn() do not change the resulting model class');
		$this->assertEquals('The Tin Drum', $book->getTitle());
		$this->assertTrue($book->getAuthor() instanceof Author, 'PropulsionObjectFormatter correctly hydrates with class');
		$this->assertEquals('Gunter', $book->getAuthor()->getFirstName(), 'PropulsionObjectFormatter correctly hydrates with class');
		$this->assertEquals('Gunter', $book->getVirtualColumn('AuthorName'), 'PropulsionObjectFormatter adds withColumns as virtual columns');
		$this->assertEquals('Grass', $book->getVirtualColumn('AuthorName2'), 'PropulsionObjectFormatter correctly hydrates all virtual columns');
	}
