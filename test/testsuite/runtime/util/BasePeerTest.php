...
class BasePeerTest extends BookstoreTestBase
{
	...
	public function testMssqlApplyLimitWithOffset()
	{
		...
		$expectedSql = "SELECT book.ID, book.TITLE, publisher.NAME, (SELECT MAX(publisher.NAME) FROM publisher WHERE publisher.ID = book.PUBLISHER_ID) AS PublisherName FROM book LEFT JOIN publisher ON (book.PUBLISHER_ID=publisher.ID) ORDER BY (SELECT NULL) OFFSET 20 ROWS FETCH NEXT 20 ROWS ONLY";
		$sql = BasePeer::createSelectSql($c, $params);
		$this->assertEquals($expectedSql, $sql);
	}
	...
	public function testMssqlApplyLimitWithOffsetOrderByAggregate()
	{
		...
		$expectedSql = "SELECT book.ID, book.TITLE, publisher.NAME, (SELECT MAX(publisher.NAME) FROM publisher WHERE publisher.ID = book.PUBLISHER_ID) AS PublisherName FROM book LEFT JOIN publisher ON (book.PUBLISHER_ID=publisher.ID) ORDER BY PublisherName DESC OFFSET 20 ROWS FETCH NEXT 20 ROWS ONLY";
		$sql = BasePeer::createSelectSql($c, $params);
		$this->assertEquals($expectedSql, $sql);
	}
	...
	public function testMssqlApplyLimitWithOffsetMultipleOrderBy()
	{
		...
		$expectedSql = "SELECT book.ID, book.TITLE, publisher.NAME, (SELECT MAX(publisher.NAME) FROM publisher WHERE publisher.ID = book.PUBLISHER_ID) AS PublisherName FROM book LEFT JOIN publisher ON (book.PUBLISHER_ID=publisher.ID) ORDER BY PublisherName DESC,book.TITLE ASC OFFSET 20 ROWS FETCH NEXT 20 ROWS ONLY";
		$sql = BasePeer::createSelectSql($c, $params);
		$this->assertEquals($expectedSql, $sql);
	}
