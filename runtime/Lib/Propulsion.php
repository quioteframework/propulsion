...
class Propulsion
{
	...
	public static function getDatabaseMap($name = null)
	{
		...
		if (!isset(self::$dbMaps[$name])) {
			...
			if (!$dbMap instanceof DatabaseMap) {
				throw new PropulsionException($clazz . ' is not a DatabaseMap');
			}
	...
	public static function initConnection($conparams, $name, ?string $defaultClass = null)
	{
		...
		try {
			...
			if (!$con instanceof PDO) {
				throw new PropulsionException($classname . ' is not a PDO instance');
			}
		...
		if (isset($conparams['attributes']) && is_array($conparams['attributes'])) {
			...
			foreach ($attributes as $key => $value) {
				if (!is_int($key)) {
					throw new PropulsionException('Invalid PDO connection attribute key specified: ' . var_export($key, true));
				}
