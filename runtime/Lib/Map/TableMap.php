...
class TableMap
{
  ...
  /**
   * Whether this table is a cross-reference table
   * @return     boolean
   */
  ...
  public function addColumn($name, $phpName, $type, $isNotNull = false, $size = null, $defaultValue = null, $pk = false, $fkTable = null, $fkColumn = null)
  {
    ...
    // Normalize the column name for consistent storage and retrieval
    $normalizedName = ColumnMap::normalizeName($name);

    if ($pk) {
      $col->setPrimaryKey(true);
      $this->primaryKeys[$normalizedName] = $col;
    }

    if ($fkTable && $fkColumn) {
      $col->setForeignKey($fkTable, $fkColumn);
      $this->foreignKeys[$normalizedName] = $col;
    }

    $this->columns[$normalizedName] = $col;
  ...
  */
   ...
   */
  public function getPrimaryStringColumn(): mixed
  {
  ...
  {
    return $this->prefix !== null && $this->prefix !== '' && strpos($data, $this->prefix) === 0;
  }
