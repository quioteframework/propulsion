...
All notable changes to Propulsion are documented in this file, and this project
adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

Sections from 2.1.0 onward are generated by [git-cliff](https://git-cliff.org)
from the commit subjects belonging to each tag -- the same output that becomes
the GitHub Release body, so the two cannot drift. The reasoning behind a change
lives in its commit body, which `git log` shows and this file does not. The
2.0.0 and 1.0.0 sections below are hand-written: their history predates this
repository's conventional-commit convention, so there is nothing for git-cliff
...
[2.1.0]: https://github.com/quioteframework/propulsion/compare/v2.0.0...v2.1.0
