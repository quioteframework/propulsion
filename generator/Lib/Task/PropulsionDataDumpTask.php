 ...
 * the PropulsionDataSQLTask class.
 *
 ...
 */
class PropulsionDataDumpTask extends AbstractPropulsionDataModelTask
{
			...
			foreach ($dataModel->getDatabases() as $database) {
				...
				if (empty($this->databaseName) || ($this->databaseName && $database->getName() == $this->databaseName)) {
					...
					try {
						...
						$this->log("PDOException while connecting to DB: ". $se->getMessage(), Project::MSG_ERR);
	 ...
	 * @return     PDOStatement
	 */
	private function getTableDataStmt($tableName, PropulsionPlatformInterface $platform)
	{
	...
	private function createXMLDoc(Database $database)
	{
		...
		$doc->appendChild($doc->createComment("Created by PropulsionDataDumpTask."));
