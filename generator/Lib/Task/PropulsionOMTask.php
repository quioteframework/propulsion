 ...
 */
class PropulsionOMTask extends AbstractPropulsionDataModelTask
{
	...
	/**
	 * Strips autogenerated timestamp lines before content comparison so a timestamp-only
	 * diff does not trigger a file rewrite.
	 */
	 ...
	 * @todo       -cPropulsionOMTask Consider refactoring build() method into AbstractPropulsionDataModelTask (would need to be more generic).
	 */
	protected function build($builder, $overwrite = true)
	{
		...
		// skip unchanged files (ignore timestamp-only diffs)
		if ($_f->exists() && $this->stripTimestamp($script) === $this->stripTimestamp($_f->contents())) {
