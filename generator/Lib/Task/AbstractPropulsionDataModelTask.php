...
/**
 * An abstract base Propulsion task to perform work related to the XML schema file.
 *
...
use \XSLTProcessor;
use \Exception;
abstract class AbstractPropulsionDataModelTask extends Task
{
	 ...
	 *
	 * @return array<string,string> data model name to database name map.
	 */
	 ...
	 * @param      Phing\Io\File $outputDirectory
	 * @return     void
	 * @throws     Exception
	 */
	...
	public function createMapper() {
		if ($this->mapperElement !== null) {
			throw new BuildException("Cannot define more than one mapper.", $this->mapperElement->getLocation());
		}
			...
			foreach ($dataModelFiles as $dmFilename) {
				...
				if ($this->validate && $this->xsdFile) {
					...
					if (!$dom->schemaValidate($this->xsdFile->getAbsolutePath())) {
						throw new EngineException("XML schema file (".$xmlFile->getPath().") does not validate. See warnings above for reasons validation failed (make sure error_reporting is set to show E_WARNING if you don't see any). Location: " . $this->getLocation());
					}
				}
	 ...
	 *
	 * @param      AppData[] $ads The datamodels to join
