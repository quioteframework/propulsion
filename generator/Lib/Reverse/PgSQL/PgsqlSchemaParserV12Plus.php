...
/**
 * Postgresql database schema parser for PostgreSQL V12 and up
 *
...
use \PDO;
use \stdClass;
class PgsqlSchemaParserV12Plus extends BaseSchemaParser
{
	...
	protected function addColumns(Table $table, $oid, $version)
	{
		...
		while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
			...
			if (((int) $row['isarray']) === 1) {
				throw new EngineException (sprintf ("Array datatypes are not currently supported [%s.%s]", $table->getName(), $row['attname']));
			} // if (((int) $row['isarray']) === 1)
