...
class OraclePlatform extends DefaultPlatform
{
	...
	public function getColumnDDL(Column $col, bool $allowIdentity = true)
	{
		...
		if ($allowIdentity && $col->isAutoIncrement() && $col->isIdentity() && $table && $this->usesNativeIdentity($table)) {
			...
			// GENERATED ... AS IDENTITY must come *before* NOT NULL -- Oracle
			// rejects the reverse order outright (ORA-03076 "unexpected item
			// GENERATED in a column definition or inline constraint"),
			// confirmed against a live gvenzl/oracle-free:23-slim container
			// while live-verifying this feature for the first time.
