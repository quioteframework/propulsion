 ...
 * @method     array<$modelClass> findBy" . $column->getPhpName() . "($phpType \$" . $column->getName() . ") Return $modelClass objects filtered by the " . $column->getName() . " column";
        ...
        foreach ($this->getTable()->getForeignKeys() as $fk) {
            $relationName = $this->getFKPhpNameAffix($fk);
            $relatedQueryClass = $this->getNewStubQueryBuilder(self::requireFkForeignTable($fk))->getFullyQualifiedClassname();
            $script .= "
 * @method     \\$relatedQueryClass use" . $relationName . "Query(?string \$relationAlias = null) Use the " . $relationName . " relation query object";
        ...
        foreach ($this->getTable()->getReferrers() as $refFK) {
            $relationName = $this->getRefFKPhpNameAffix($refFK);
            $relatedQueryClass = $this->getNewStubQueryBuilder(self::requireFkLocalTable($refFK))->getFullyQualifiedClassname();
            $script .= "
 * @method     \\$relatedQueryClass use" . $relationName . "Query(?string \$relationAlias = null) Use the " . $relationName . " relation query object";
        }
 ...
 *";
        foreach ($this->getTable()->getForeignKeys() as $fk) {
            $relationName = $this->getFKPhpNameAffix($fk);
            $relatedModelClass = $this->getNewStubObjectBuilder(self::requireFkForeignTable($fk))->getFullyQualifiedClassname();
            $script .= "
 ...
 * @method     array<$modelClass> findBy" . $relationName . "(\\$relatedModelClass \$" . lcfirst($relationName) . ") Return $modelClass objects filtered by the " . $relationName . " relation";
        ...
        foreach ($this->getTable()->getReferrers() as $refFK) {
            $relationName = $this->getRefFKPhpNameAffix($refFK);
            $relatedModelClass = $this->getNewStubObjectBuilder(self::requireFkLocalTable($refFK))->getFullyQualifiedClassname();
            $script .= "
 * @method     ?$modelClass findOneBy" . $relationName . "(\\$relatedModelClass \$" . lcfirst($relationName) . ") Return the first $modelClass filtered by the " . $relationName . " relation
 * @method     array<$modelClass> findBy" . $relationName . "(\\$relatedModelClass \$" . lcfirst($relationName) . ") Return $modelClass objects filtered by the " . $relationName . " relation";
        }
...
abstract class ".$this->getClassname()." extends " . $parentClass . "
{
     ...
     */
    public function __construct(?string \$dbName = '" . (self::requireDatabase($this->getTable())->getName() ?? '(unnamed)') . "', ?string \$modelName = '" . addslashes($this->getStubObjectBuilder()->getFullyQualifiedClassname()) . "', ?string \$modelAlias = null)
    {
...
";
    ...
    protected function addConstructorOpen(&$script): void
    {
        $table = $this->getTable();
        $script .= "
    public function __construct(\$dbName = '" . (self::requireDatabase($table)->getName() ?? '(unnamed)') . "', \$modelName = '" . addslashes($this->getNewStubObjectBuilder($table)->getFullyQualifiedClassname()) . "', \$modelAlias = null)
    {";
    }
...
";
    ...
    protected function addFilterByFk(&$script, ForeignKey $fk): void
    {
        ...
        $table = $this->getTable();
        $queryClass = $this->getStubQueryBuilder()->getClassname();
        $fkTable = self::requireFkForeignTable($fk);
    ...
    public function filterBy$relationName($objectName, \$comparison = null)
    {
        ...
        foreach ($fk->getLocalForeignMapping() as $localColumn => $foreignColumn) {
            ...
            $foreignColumnObject = self::requireNotNull($fkTable->getColumn($foreignColumn), sprintf("Column '%s' on table '%s'", $foreignColumn, $fkTable->getName() ?? '(unnamed)'));
            $script .= "
...
";
    ...
    protected function addFilterByRefFk(&$script, ForeignKey $fk): void
    {
        ...
        $table = $this->getTable();
        $queryClass = $this->getStubQueryBuilder()->getClassname();
        $fkTable = self::requireFkLocalTable($fk);
        $fkStubObjectBuilder = $this->getNewStubObjectBuilder($fkTable);
        $this->declareClassFromBuilder($fkStubObjectBuilder);
        $fkPhpName = $fkStubObjectBuilder->getClassname();
...
";
    ...
    {
        $queryClass = $this->getStubQueryBuilder()->getClassname();
        $crossRefTable = self::requireFkLocalTable($crossFK);
        $foreignTable = self::requireFkForeignTable($crossFK);
