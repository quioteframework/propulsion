...
class NestedSetPeerBuilder extends AbstractPeerBuilder
{
	...
	{
		return $this->getStringBuildProperty('basePrefix') . $this->getStubObjectBuilder()->getUnprefixedClassname() . 'NestedSetPeer';
	}
	...
	protected function addConstants(&$script): void
	{
		...
		foreach ($table->getColumns() as $col) {
			if ($col->isNestedSetLeftKey()) {
				$colname['left'] = $tableName . '.' . strtoupper(self::requireNotNull($col->getName(), sprintf("Column '%s' name", $col->getConstantName())));
			}

			if ($col->isNestedSetRightKey()) {
				$colname['right'] = $tableName . '.' . strtoupper(self::requireNotNull($col->getName(), sprintf("Column '%s' name", $col->getConstantName())));
			}

			if ($col->isTreeScopeKey()) {
				$colname['scope'] = $tableName . '.' . strtoupper(self::requireNotNull($col->getName(), sprintf("Column '%s' name", $col->getConstantName())));
			}
