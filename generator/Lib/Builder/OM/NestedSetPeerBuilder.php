...
class NestedSetPeerBuilder extends AbstractPeerBuilder
{
	...
	{
		return $this->getBuildPropertyString('basePrefix') . $this->getStubObjectBuilder()->getUnprefixedClassname() . 'NestedSetPeer';
	}
	...
	protected function addConstants(&$script): void
	{
		...
		foreach ($table->getColumns() as $col) {
			if ($col->isNestedSetLeftKey()) {
				$colname['left'] = $tableName . '.' . strtoupper((string) $col->getName());
			}

			if ($col->isNestedSetRightKey()) {
				$colname['right'] = $tableName . '.' . strtoupper((string) $col->getName());
			}

			if ($col->isTreeScopeKey()) {
				$colname['scope'] = $tableName . '.' . strtoupper((string) $col->getName());
			}
