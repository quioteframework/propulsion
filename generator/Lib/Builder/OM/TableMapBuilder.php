...
";
     ...
     */
    public const DATABASE_NAME = '" . ($this->requireDatabase()->getName() ?? '') . "';
...
";
     ...
     */
    public const " . strtoupper((string) $col->getName()) . "_NUM = " . $colIdx . ";
";
            $colIdx++;
        }
    }
    ...
    public static array \$fieldNames = [";
        ...
        foreach ($table->getColumns() as $col) {
            ...
            $script .= "\n        '" . strtoupper((string) $col->getName()) . "' => " . (array_search($col, $table->getColumns()));
            $first = false;
        }
    ...
    public static array \$fieldKeys = [";
        ...
        foreach ($table->getColumns() as $col) {
            ...
            $script .= "\n        " . (array_search($col, $table->getColumns())) . " => '" . strtoupper((string) $col->getName()) . "'";
            $first = false;
        }
    ...
    public function initialize(): void
    {
            ...
            if ($col->isPrimaryKey()) {
                if ($col->isForeignKey()) {
                    foreach ($col->getForeignKeys() as $fk) {
                        $script .= "
        \$this->addForeignPrimaryKey('$cup', '$cfc', '".$col->getType()."' , '".$fk->getForeignTableName()."', '".strtoupper((string) $fk->getMappedForeignColumn((string) $col->getName()))."', ".($col->isNotNull() ? 'true' : 'false').", ".$size.", $default);";
                    }
                } else {
                    $script .= "
        \$this->addPrimaryKey('$cup', '$cfc', '".$col->getType()."', ".var_export($col->isNotNull(), true).", ".$size.", $default);";
                ...
                if ($col->isForeignKey()) {
                    foreach ($col->getForeignKeys() as $fk) {
                        $script .= "
        \$this->addForeignKey('$cup', '$cfc', '".$col->getType()."', '".$fk->getForeignTableName()."', '".strtoupper((string) $fk->getMappedForeignColumn((string) $col->getName()))."', ".($col->isNotNull() ? 'true' : 'false').", ".$size.", $default);";
                    }
                } else {
                    $script .= "
...
";
    ...
    protected function addBuildRelations(&$script): void
    {
        ...
        foreach ($this->getTable()->getForeignKeys() as $fkey) {
            $relatedClasses[] = $this->getNewStubObjectBuilder($this->requireForeignTable($fkey))->getFullyQualifiedClassname();
        }
        foreach ($this->getTable()->getReferrers() as $fkey) {
            $relatedClasses[] = $this->getNewStubObjectBuilder($this->requireFkOwningTable($fkey))->getFullyQualifiedClassname();
        }
        foreach ($this->getTable()->getCrossFks() as $fkList) {
            list($refFK, $crossFK) = $fkList;
            $relatedClasses[] = $this->getNewStubObjectBuilder($this->requireForeignTable($crossFK))->getFullyQualifiedClassname();
        }
    ...
    {";
        foreach ($this->getTable()->getForeignKeys() as $fkey) {
            ...
            $relatedClassName = $this->getNewStubObjectBuilder($this->requireForeignTable($fkey))->getClassname();
            $script .= "
        ...
        foreach ($this->getTable()->getReferrers() as $fkey) {
            ...
            $onDelete = $fkey->hasOnDelete() ? "'" . $fkey->getOnDelete() . "'" : 'null';
            $onUpdate = $fkey->hasOnUpdate() ? "'" . $fkey->getOnUpdate() . "'" : 'null';
            $relatedClassName = $this->getNewStubObjectBuilder($this->requireFkOwningTable($fkey))->getClassname();
            $script .= "
        ...
        foreach ($this->getTable()->getCrossFks() as $fkList) {
            ...
            $relatedClassName = $this->getNewStubObjectBuilder($this->requireForeignTable($crossFK))->getClassname();
            $script .= "
