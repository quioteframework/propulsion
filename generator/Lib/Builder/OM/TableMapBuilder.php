...
{
    /**
     * The generated code upper-cases column names to build constant names
     * (e.g. self::ID). Column::getName() is nullable only while a schema is
     * still being parsed; by code-generation time every column is named.
     */
    private static function upperColumnName(Column $col): string
    {
        return strtoupper(self::requireNotNull($col->getName(), sprintf("Column '%s' name", $col->getConstantName())));
    }
	...
	public function getNamespace()
	{
		if ($namespace = parent::getNamespace()) {
			return $namespace . '\\' . $this->getMapPackageSegment();
		}
		return null;
	}
    ...
    {
        $table = $this->getTable();
        return self::requireNotNull($table->getPhpName(), sprintf("Table '%s' PHP name", $table->getName() ?? '(unnamed)')) . 'TableMap';
    }
     ...
     * ObjectBuilder::getUseStatements() for the full rationale (identical logic, copied
     * here because this builder needed the same fix): without this, addClassBody()'s FQCN
     * declareClass() calls for core Propulsion\* classes either get a redundant `use`
     * emitted for a flat/non-namespaced class (fatal -- "name is already in use" -- when
     * PropulsionQuickBuilder concatenates many such classes into one eval()'d script with
     * no namespace block between them), or, for a genuinely namespaced class, would need
...
";
     ...
     */
    public const DATABASE_NAME = '" . (self::requireDatabase($table)->getName() ?? '(unnamed)') . "';
...
";
        ...
        foreach ($table->getColumns() as $col) {
            $colName = self::upperColumnName($col);
            $script .= "
    /**
     * the column name for the " . $colName . " field
     */
    public const " . $colName . " = '" . $table->getName() . '.' . $col->getName() . "';
";
            $colIdx++;
        }
     ...
     */
    public const " . self::upperColumnName($col) . "_NUM = " . $colIdx . ";
";
            $colIdx++;
        }
    }
    ...
    public static array \$fieldNames = [";
        ...
        foreach ($table->getColumns() as $col) {
            ...
            $script .= "\n        '" . self::upperColumnName($col) . "' => " . (array_search($col, $table->getColumns()));
            $first = false;
        }
    ...
    public static array \$fieldKeys = [";
        ...
        foreach ($table->getColumns() as $col) {
            ...
            $script .= "\n        " . (array_search($col, $table->getColumns())) . " => '" . self::upperColumnName($col) . "'";
            $first = false;
        }
    ...
    public function initialize(): void
    {
            ...
            if ($col->isPrimaryKey()) {
                if ($col->isForeignKey()) {
                    foreach ($col->getForeignKeys() as $fk) {
                        $script .= "
        \$this->addForeignPrimaryKey('$cup', '$cfc', '".$col->getType()."' , '".$fk->getForeignTableName()."', '".strtoupper(self::requireNotNull($fk->getMappedForeignColumn($colName), sprintf("Foreign key '%s' mapped foreign column for '%s'", $fk->getName() ?? '(unnamed)', $colName)))."', ".($col->isNotNull() ? 'true' : 'false').", ".$size.", $default);";
                    }
                } else {
                    $script .= "
        \$this->addPrimaryKey('$cup', '$cfc', '".$col->getType()."', ".var_export($col->isNotNull(), true).", ".$size.", $default);";
                ...
                if ($col->isForeignKey()) {
                    foreach ($col->getForeignKeys() as $fk) {
                        $script .= "
        \$this->addForeignKey('$cup', '$cfc', '".$col->getType()."', '".$fk->getForeignTableName()."', '".strtoupper(self::requireNotNull($fk->getMappedForeignColumn($colName), sprintf("Foreign key '%s' mapped foreign column for '%s'", $fk->getName() ?? '(unnamed)', $colName)))."', ".($col->isNotNull() ? 'true' : 'false').", ".$size.", $default);";
                    }
                } else {
                    $script .= "
        ...
        foreach ($table->getValidators() as $val) {
            $col = self::requireNotNull($val->getColumn(), "Validator's column");
            $cup = self::upperColumnName($col);
...
";
    ...
    protected function addBuildRelations(&$script): void
    {
        ...
        foreach ($this->getTable()->getForeignKeys() as $fkey) {
            $relatedClasses[] = $this->getNewStubObjectBuilder(self::requireFkForeignTable($fkey))->getFullyQualifiedClassname();
        }
        foreach ($this->getTable()->getReferrers() as $fkey) {
            $relatedClasses[] = $this->getNewStubObjectBuilder(self::requireFkLocalTable($fkey))->getFullyQualifiedClassname();
        }
        foreach ($this->getTable()->getCrossFks() as $fkList) {
            list($refFK, $crossFK) = $fkList;
            $relatedClasses[] = $this->getNewStubObjectBuilder(self::requireFkForeignTable($crossFK))->getFullyQualifiedClassname();
        }
    ...
    {";
        foreach ($this->getTable()->getForeignKeys() as $fkey) {
            ...
            $relatedClassName = $this->getNewStubObjectBuilder(self::requireFkForeignTable($fkey))->getClassname();
            $script .= "
        ...
        foreach ($this->getTable()->getReferrers() as $fkey) {
            ...
            $onDelete = $fkey->hasOnDelete() ? "'" . $fkey->getOnDelete() . "'" : 'null';
            $onUpdate = $fkey->hasOnUpdate() ? "'" . $fkey->getOnUpdate() . "'" : 'null';
            $relatedClassName = $this->getNewStubObjectBuilder(self::requireFkLocalTable($fkey))->getClassname();
            $script .= "
        ...
        foreach ($this->getTable()->getCrossFks() as $fkList) {
            ...
            $relatedClassName = $this->getNewStubObjectBuilder(self::requireFkForeignTable($crossFK))->getClassname();
            $script .= "
